...
- **The one hard rule:** no refactor commit may change a computed number. The
  arbiter is `tests/equivalence/`, and it reported **IDENTICAL** on every commit
  that touched code. Two evaluator *defaults* changed, both signed off at a
...
7. **D14's compiler filter.** `scripts/compile_results.py` still excludes the
   headline Houston run as a scratch run. Changing the filter needs sign-off.
8. **Table 3 reproduction gaps.** The twelve 64×64 frozen cells and the three
...
9. **The 20-epoch observation, and a discrepancy inside it.** Phase 7's smoke
   ...
   not cleanup.
10. **Doc filenames.** The phase-8 plan named `docs/{pretraining,evaluation,
...
11. mypy `ignore_errors` on 16 modules (175 findings, all typing friction — the
...
12. `docs/presentation/` stays in the tree with SUPERSEDED banners — the
    manifest's verdict is `keep`, and `RESULTS.json` is left byte-for-byte as
    the compiler wrote it.
13. Legacy vocabulary survives exactly where `CHANGES.md`'s "Where the retired
    ...
    retirement.
14. `scripts/reproduce/run_eval{,_trento}.sh` and
