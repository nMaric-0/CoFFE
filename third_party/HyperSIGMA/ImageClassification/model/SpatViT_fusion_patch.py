...
except ImportError:  # pragma: no cover - mmengine optional
    from ._compat import get_dist_info
...
class SpatViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=1, in_chans=3, num_classes=80, embed_dim=768, depth=12,
        ...
        self.cls = nn.Conv2d(embed_dim*4,num_classes,kernel_size=1,stride=1,padding = 0,bias=True)
        self.classifier1 = nn.Sequential(
            nn.Linear(
                in_features=768*4,#196608
                out_features=128
            ),
            nn.Linear(
                in_features=128,
        ...
        # Local patch (CoFFE): add k=3 case so small-input HSI deployment
        # can use a 3x3 patch_embed; all four FPN stages keep the native
        # token grid (Identity), matching upstream's k=11 handling.
        if patch_size == 3:
            self.fpn1 = nn.Identity()
