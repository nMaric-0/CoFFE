...
These are real checkpoints produced by `scripts.pretrain_enhanced.run_pretrain`
on the synthetic `houston_mini` scene with the headline `simmim_token`
objective, 3 epochs, on the **pre-refactor** code. They are the G4 artefact:
...
`scripts/evaluate_cosine.py:fix_state_dict_keys` consumes. Optimiser and
