...
class PatchedEpisodeSampler:
    ...
    def _select_fixed_support_indices(self) -> Dict[int, List[int]]:
        ...
        for cls in self.available_classes:
            ...
            fixed_support[cls] = self.rng.choice(
                indices,
                self.k_shot,
                replace=False
            ).tolist()
    ...
    def _build_class_indices(self) -> Dict[int, List[int]]:
        ...
        if hasattr(self.dataset, 'labels'):
            ...
            for idx in range(len(self.dataset)):
                ...
                label = int(sample['label'].item() if isinstance(sample['label'], torch.Tensor)
                           else sample['label'])
                if label not in class_indices:
                    class_indices[label] = []
                class_indices[label].append(idx)
    ...
    def get_class_info(self) -> Dict:
        ...
        return {
            ...
            "samples_per_class": {c: len(self.class_indices[c]) 
                                  for c in self.available_classes},
            "min_samples": min(len(self.class_indices[c]) 
                              for c in self.available_classes),
        }
...
class CrossDatasetEpisodeSampler:
    ...
    def __init__(
        ...
        self.source_dataset = ConcatDataset(source_datasets) if len(source_datasets) > 1 else source_datasets[0]
        ...
        self.available_classes = [
            ...
            if (len(self.source_class_indices.get(c, [])) >= k_shot and
                len(self.target_class_indices.get(c, [])) >= k_query)
        ]
