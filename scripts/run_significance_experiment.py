...
Usage:
    python scripts/run_significance_experiment.py --stage all --dry-run
    python scripts/run_significance_experiment.py --stage all \
        --groups hsi_only enhanced_mae mft_mae mft_spatial
    python scripts/run_significance_experiment.py --stage train --groups mft_mae
"""
...
from scripts import sig_significance_config as cfg  # noqa: E402
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        if args.stage in ("aggregate", "all"):
            print(f"\n=== AGGREGATE ===\n  $ {sys.executable} "
                  f"{REPO_ROOT / 'scripts' / 'aggregate_significance.py'}")
    ...
    if args.stage in ("aggregate", "all"):
        ...
        agg = [sys.executable, str(REPO_ROOT / "scripts" / "aggregate_significance.py")]
        if args.groups:
