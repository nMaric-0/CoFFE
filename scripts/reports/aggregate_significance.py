...
def _read_metric_means(name: str) -> dict[str, float] | None:
...
def main() -> int:
    ...
    print(f"\n# Significance report (epoch {cfg.EPOCHS}, {len(cfg.SEEDS)} seeds: {cfg.SEEDS})\n")
    for group in groups:
        print(f"\n## {group}\n")
        print(
            "| Dataset | Variant | n | OA (mean±std, 95%CI) | "
