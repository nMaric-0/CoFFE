"""
MFT Pixel Tokenization Backbone.

Adapted from: https://github.com/srinadh99/Transformer-Models-for-Multimodal-Remote-Sensing-Data

This backbone uses pixel-wise (spatial) attention tokenization for the
auxiliary modality (LiDAR), creating a fusion token that serves as CLS.

Key difference from MFTChannelBackbone:
- Pixel tokenization uses spatial attention (position-based)
- Channel tokenization uses feature attention (channel-based)

According to MFT paper, channel tokenization performs better (98.05% vs 95.47%
on Trento), but pixel tokenization may be better for certain spatial patterns.
"""
import torch
...
class MFTPixelBackbone(BackboneBase):
    """
    MFT backbone with pixel tokenization for multimodal fusion.

    Architecture:
        HSI: Conv3D -> HetConv -> Flatten -> LearnableTokenizer -> N tokens
        LiDAR: Conv2D -> Flatten -> PixelTokenizer -> 1 token (CLS)
        Fusion: Concat [CLS, HSI_tokens] -> MFTEncoder -> (patch_emb, cls_emb)

    The key difference from MFTChannelBackbone is the tokenization type
    for the auxiliary modality:
    - Channel: attention weights depend on feature channels
    - Pixel: attention weights are spatial-agnostic (same weight per position)
    ...
    def __init__(
        ...
        # Learnable tokenization for HSI (channel type)
        ...
        # PIXEL tokenization for LiDAR -> 1 token (serves as CLS)
        # This is the key difference from MFTChannelBackbone
        self.aux_tokenizer = LearnableTokenizer(
            input_dim=self.fm * 4,
            embed_dim=embed_dim,
            num_tokens=1,
            token_type="pixel"  # <-- Pixel tokenization
        )
    ...
    def forward_tokens_only(
        ...
        aux_features = self.extract_aux_features(aux)

        hsi_tokens = self.hsi_tokenizer(hsi_features)
        cls_token = self.aux_tokenizer(aux_features)
    ...
    def forward_features(
        ...
        # Tokenize
        hsi_tokens = self.hsi_tokenizer(hsi_features)
        cls_token = self.aux_tokenizer(aux_features)  # Pixel tokenization

        # Concatenate: [CLS, HSI_tokens]
        tokens = torch.cat([cls_token, hsi_tokens], dim=1)
