...
Mirrors the public surface of ``MFTCPEACosine`` so the existing
``scripts/evaluate_cosine.py`` episode loop can drive it. The wrapper:
...
* reuses ``compute_prototypes`` / ``_forward_mean_features`` /
  ``_forward_mean_distances`` from ``MFTCPEACosine`` verbatim (we don't
...
class HyperSIGMACosine(nn.Module):
    ...
    def adapt_embeddings(
        ...
        # No CPEA adaptation in HyperSIGMA — pass patch tokens through.
        return patch_emb

    # ------------------------------------------------------------------
    # Prototype math (copy of MFTCPEACosine internals; no shared state)
    # ------------------------------------------------------------------
