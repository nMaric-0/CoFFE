...
2. **Writers emit canonical only.** The one deliberate exception is
   ``scripts/aggregate_significance.py``, whose emitted ``group``/``variant``
...
``scripts/sig_significance_config.py``). Those are frozen names of files that
...
importing a ``normalize_*`` helper never pulls in ``torch``::
    ...
    from coffe_compat import MFTCPEACosine   # -> models.coffe.CoFFE + warning

Post-phase-5 this module becomes ``coffe/compat.py``; the public names stay.
"""
...
LEGACY_CLASSES: Dict[str, Tuple[str, str]] = {
    "MFTCPEACosine": ("models.coffe", "CoFFE"),
    "MFTOriginalCosine": ("models.mft_original", "MFTOriginal"),
    "HyperSIGMACosine": ("models.hypersigma.few_shot", "HyperSIGMAFewShot"),
    "EnhancedMaskedSpectralSpatialModel": ("pretrain.simmim", "SimMIMPretrainModel"),
}
