...
class MockDataset:
    ...
    def extract_patch(self, y, x):
        ...
        return (torch.from_numpy(hsi).permute(2, 0, 1), torch.from_numpy(aux).permute(2, 0, 1))


class TestEpisodeSampler:
    def test_sample_episode(self):
        dataset = MockDataset()
        sampler = EpisodeSampler(dataset, n_way=3, k_shot=5, k_query=10, num_episodes=10)
