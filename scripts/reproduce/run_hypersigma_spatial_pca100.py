...
``scripts/reports/aggregate_experiment_results.py`` picks the results up.

Usage (one dataset):
    python scripts/reproduce/run_hypersigma_spatial_pca100.py --dataset houston --device cuda:1

Typically driven for all three datasets by
``scripts/reproduce/run_hypersigma_spatial_pca100.sh``.
"""
