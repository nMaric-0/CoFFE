"""
CoFFE — Cross-modal Fusion Feature Encoder (the paper's compact encoder).

HSI and auxiliary (LiDAR) bands are concatenated **at the input** and jointly
embedded — the paper's key architectural claim against MFT's external fusion
token. The encoder is a pre-LayerNorm transformer over 121 pixel tokens plus one
prepended class-agnostic token (PAPER_CANON §2).

The encoder is frozen at evaluation: there are no learnable similarity
parameters, and episodes are classified by nearest class mean over pooled patch
tokens. The projection head is a pretraining-only part and is off at eval
(``use_projection=False`` in every paper run).
...
Usage:
    ...
    model.load_state_dict(checkpoint)   # pre-rename checkpoints load unchanged

The live evaluation path is in ``coffe/eval/episodic.py`` (a thin
``scripts/evaluate.py`` calls into it), which calls
:meth:`CoFFE.eval_patch_embeddings` and pools its output.
:meth:`forward_episode` below is a second, unexecuted copy of that maths kept
for API compatibility (PAPER_CANON §8 D3 / audit R10) — it is not what produced
any paper number.
"""
...
class CoFFE(nn.Module):
    """
    Cross-modal Fusion Feature Encoder for few-shot HSI+LiDAR classification.

    Frozen-encoder evaluation with no learnable similarity parameters;
    checkpoints from before the phase-4 rename load unchanged, because every
    ``nn.Module`` attribute name here is frozen (PAPER_CANON §7.2).

    Combines:
    - Input-level HSI+LiDAR fusion: all bands share one per-pixel token
    - A class-agnostic token, folded back into every patch token at eval
      (:meth:`eval_patch_embeddings`, PAPER_CANON §8 D3)
    - Nearest-class-mean classification over the pooled patch tokens
    ...
    Args:
        ...
        cls_token_weight: Weight of the class-agnostic token added to every
                 patch embedding at eval time (the eval feature is
                 z = mean_j(patch_emb_j) + cls_token_weight * cls_emb).
                 Config/CLI still spell this `lambda_factor` - see
                 :meth:`eval_patch_embeddings`.
        ...
        temperature: Logit scaling on the cosine path (default: 10.0). The paper
            protocol is Euclidean nearest-class-mean, which never consults it
            (PAPER_CANON §4).
    ...
    def __init__(
        ...
        if pool_sigma is not None:
            self.register_buffer(
                "_center_pool_weights", make_center_weights(patch_size, pool_sigma, normalize=True)
            )
        ...
        self.channel_tokenizer = ChannelTokenizer(
            in_channels=self.total_channels, embed_dim=embed_dim
        )
        self.spatial_tokenizer = SpatialTokenizer(embed_dim=embed_dim)

        # ========== Class-Agnostic Token ==========
        self.class_agnostic_emb = nn.Parameter(torch.randn(1, 1, embed_dim) * 0.02)
        ...
        # NOTE: No DenseSimilarity - the classifier is a distance to class means
    ...
    def forward_features(
        self, hsi: torch.Tensor, aux: torch.Tensor | None = None
    ...
    def eval_patch_embeddings(
        ...
        """Add the weighted class-agnostic token to every patch embedding.

            z̄ⱼ = zⱼ + w · z_cls

        **This is the live eval path** (PAPER_CANON §8 D3). The evaluator pools
        the tensor returned here, so with the paper's ``w = 0.5`` the feature a
        prototype is built from is

            z = mean_j(patch_emb_j) + 0.5 · cls_emb

        — not "patch tokens pooled" as the paper's §3 describes it. The
        behaviour is frozen; D3 records the gap and Nikola's decision to keep it.

        The ``renormalize`` branch below is **inert at eval time**: every paper
        run sets ``use_projection=False``, which makes ``self.projection`` an
        ``nn.Identity`` with no ``l2_normalize`` attribute.

        Args:
            patch_emb: [B, N, D] patch embeddings
            cls_emb: [B, D] class-agnostic token embedding
            cls_token_weight: override for ``self.cls_token_weight``
            renormalize: re-apply L2 normalization (inert unless the projection
                head is on and L2-normalizing)

        Returns:
            [B, N, D] patch embeddings with the class token folded in
        """
        if cls_token_weight is None:
            cls_token_weight = self.cls_token_weight

        adapted = patch_emb + cls_token_weight * cls_emb.unsqueeze(1)

        if (
            renormalize
            and hasattr(self.projection, "l2_normalize")
            and self.projection.l2_normalize
        ):
    ...
    def compute_prototypes(self, features: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        """
        Compute the per-class mean of the support features.
    ...
    def forward_episode(
        ...
        support_aux: torch.Tensor | None,
        support_labels: torch.Tensor,
        query_hsi: torch.Tensor,
        query_aux: torch.Tensor | None = None,
    ) -> torch.Tensor:
        """
        Forward pass for one few-shot episode (nearest class mean).

        **Dead on the paper path** (PAPER_CANON §8 D3 / audit R10): the live
        evaluator in ``scripts/evaluate.py`` re-implements these steps inline so
        it can capture intermediate embeddings. Kept for API compatibility.
        ...
        # Adapt embeddings
        s_adapted = self.eval_patch_embeddings(s_patch, s_cls)
        q_adapted = self.eval_patch_embeddings(q_patch, q_cls)
    ...
    def _forward_mean_features(
        self, s_features: torch.Tensor, support_labels: torch.Tensor, q_features: torch.Tensor
    ) -> torch.Tensor:
        """
