...
- SimMIMPretrainModel supports band-only, spatial-only, and
...
def test_simmim_model_band_only():
    ...
    _loss, info = _run_and_check(pretrain_model)
...
def test_simmim_model_spatial_only():
...
def test_simmim_model_combined():
