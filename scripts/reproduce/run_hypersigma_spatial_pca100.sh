...
# results.json, plots), so scripts/reports/aggregate_experiment_results.py picks it up.
#
# Usage:
#   bash scripts/reproduce/run_hypersigma_spatial_pca100.sh                 # all three datasets
#   DEVICE=cuda:1 EPOCHS=2000 bash scripts/reproduce/run_hypersigma_spatial_pca100.sh
#   EPOCHS=20 bash scripts/reproduce/run_hypersigma_spatial_pca100.sh houston   # smoke test
#
...
for ds in "${DATASETS[@]}"; do
    ...
    echo "--------------------------------------------------------------"
    "${PYTHON}" scripts/reproduce/run_hypersigma_spatial_pca100.py \
        --dataset "${ds}" \
        --device "${DEVICE}" \
        --epochs "${EPOCHS}" \
        --lr "${LR}" \
...
echo "Aggregate with: ${PYTHON} scripts/reports/aggregate_experiment_results.py"
echo "=============================================================="
