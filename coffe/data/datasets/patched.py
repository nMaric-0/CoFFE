...
class PatchedMultimodalDataset(ABC, Dataset):
    ...
    def __init__(
        ...
        # In-place ndarray -> tensor conversion; the attributes are tensors
        # from here on (mypy only sees the ndarray they were loaded as).
        self.hsi = torch.from_numpy(self.hsi).float()  # type: ignore[assignment]
        self.aux = torch.from_numpy(self.aux).float()  # type: ignore[assignment]
        self.labels = torch.from_numpy(self.labels).long()  # type: ignore[assignment]
    ...
    def _load_split(self, base_path: str, suffix: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
